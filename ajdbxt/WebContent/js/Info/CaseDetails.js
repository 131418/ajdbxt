	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			$.each(case1, function(k, v) {
				...
				else if (k == "process_accpet_right") {
...
// 改变延长传唤
function buildCase_chose(even) {
	var sex = document.getElementById("subpoena");
	sex.value = even.value;
	return sex.value;
}
function buildCase_nochose(even) {
	var sex = document.getElementById("subpoena");
	sex.value = even.value;
	return sex.value;
}
...
// CaseDetails.jsp中的1提交
function case_relive() {
	$.confirm({
		title : '提交!',
		content : '确定提交么!',
		buttons : {
